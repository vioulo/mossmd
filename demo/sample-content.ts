...
function callout(rng: () => number): string {
  const types = ['NOTE', 'TIP', 'WARNING', 'IMPORTANT', 'QUESTION'];
  const type = types[Math.floor(rng() * types.length)];
  return [
    `> [!${type}] ` + words(rng, 3 + Math.floor(rng() * 4)),
    '> ' + words(rng, 12 + Math.floor(rng() * 16)) + '.',
    '> ' + words(rng, 10 + Math.floor(rng() * 12)) + '.',
  ].join('\n');
}
...
function section(
  ...
  for (let i = 0; i < blockCount; i++) {
    ...
    else if (pick < 0.93) parts.push(callout(rng));
    else if (pick < 0.97) parts.push(quote(rng));
...
export function generateSampleMarkdown(
  ...
  sections.push(
    ...
    [
      ...
      '> This block is installed through `customSyntax`, not hard-coded into the editor shell.',
      '> Click into the first line to reveal the original `> [!NOTE]` marker.',
    ].join('\n'),
  );
  ...
  sections.push(
    ...
    [
      ...
      "import { DownloadCloud, Paperclip, ScanEye } from 'lucide-react';",
      '',
      '<MossMD',
      '  markdownSource={markdown}',
      '  icons={{',
      '    image: {',
      '      preview: mossLucideIcon(ScanEye, { size: 16 }),',
      '    },',
      '    file: {',
      ...
      "        id: 'attach',",
      "        label: 'Attach file',",
      '        icon: mossLucideIcon(Paperclip),',
      '        apply: insertAttachment,',
      '      },',
      '    ],',
      '  }}',
      '/>',
