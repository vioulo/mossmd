...
export function MossEditor({
  ...
  const collabGenerationRef = useRef(0);
  ...
  const attachCollabAdapter = async (
    adapter: CollabAdapter,
    view: EditorView,
  ): Promise<void> => {
    const generation = ++collabGenerationRef.current;
    const previous = collabAdapterRef.current;
    if (previous === adapter && collabUnsubscribeRef.current) return;

    collabUnsubscribeRef.current?.();
    collabUnsubscribeRef.current = null;

    if (previous !== noopCollabAdapter && previous !== adapter) {
      await previous.detach();
    }

    if (viewRef.current !== view || collabGenerationRef.current !== generation) {
      if (adapter !== noopCollabAdapter) await adapter.detach();
      return;
    }

    collabAdapterRef.current = adapter;
    collabUnsubscribeRef.current = adapter.onRemoteChange((doc) => {
      if (
    ...
    try {
      await adapter.attach(view);
    } catch (error) {
      if (
        collabGenerationRef.current === generation &&
        collabAdapterRef.current === adapter
      ) {
        collabUnsubscribeRef.current?.();
        collabUnsubscribeRef.current = null;
        collabAdapterRef.current = noopCollabAdapter;
      }
      throw error;
    }
  };
  ...
  useEffect(() => {
    ...
    const registeredSyntax = registerMossSyntax(customSyntax);
      ...
      state: EditorState.create({
        ...
        extensions: [
          ...
          markdown({
            ...
            extensions: [
              highlightMarkdown,
              ...registeredSyntax.markdownExtensions,
            ],
          }),
          ...
          ...(hasWikiLinksConfig
            ? [
                wikiLinks({
                  ...wikiLinksConfig,
                  onOpen: wikiLinksConfig.onOpen ?? handleLinkClick,
                }),
              ]
            : []),
    ...
    return () => {
      ...
      collabGenerationRef.current++;
      collabUnsubscribeRef.current?.();
      collabUnsubscribeRef.current = null;
      const activeAdapter = collabAdapterRef.current;
      collabAdapterRef.current = noopCollabAdapter;
      if (activeAdapter !== noopCollabAdapter) void activeAdapter.detach();
      view.destroy();
  ...
  useEffect(() => {
    ...
    editorHandleRef.current = {
      ...
      setCollabAdapter: async (adapter) => {
        const view = viewRef.current;
        if (!view) return;
        await attachCollabAdapter(adapter, view);
      },
    };
    return () => {
