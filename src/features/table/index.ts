...
// (at the table's word boundary), CM6's word-range handling
...
function getCellSource(cell: HTMLElement): HTMLElement | null {
  return cell.querySelector<HTMLElement>('.cm-moss-table-cell-source');
}
...
function refreshCellPreview(cell: HTMLElement): void {
  const existing = cell.querySelector<HTMLElement>('.cm-moss-table-cell-preview');
...
function makeCell(
  ...
  source.addEventListener('click', (event) => {
    ...
    const url = hit.closest<HTMLElement>('.cm-moss-link-wrap')?.dataset.url;
    if (!url) return;
    event.preventDefault();
    event.stopPropagation();
