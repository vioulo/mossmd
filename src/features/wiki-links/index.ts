...
export type MossWikiLinksConfig = WikiLinksConfig;
...
class WikiLinkWidget extends WidgetType {
  ...
  override toDOM(): HTMLElement {
    ...
    span.className = `cm-moss-wiki-link cm-moss-wiki-link-${this.status}`;
...
export const mossWikiLinks = wikiLinks;
  ...
  for (const link of links) {
    ...
    if (link.label && link.labelFrom != null && link.labelTo != null && link.labelFrom < link.labelTo) {
      ...
      builder.add(
        ...
        link.labelTo,
        Decoration.mark({
          class: 'cm-moss-wiki-link cm-moss-wiki-link-resolved',
